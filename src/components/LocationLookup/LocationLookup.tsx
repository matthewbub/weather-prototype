...
import {
	...
	locationLookupApiUrls,
} from "./LocationLookup.constants";
...
export const LocationLookupForm = () => {
	...
	const setSelectedLocationToNull = store(
		(state) => state.setSelectedLocationToNull,
	);
	...
	const fetchSearchResults = async (value: string) => {
		...
		const searchResultsData = await fetch(
			locationLookupApiUrls.autoComplete.url,
			{
				method: "POST",
				body: JSON.stringify({
					type: "city",
		...
		if (
			searchResults?.find(
				(result: { formatted: string }) =>
					result.formatted === locationInputValue,
			)
		) {
			return;
		}
		...
		<div className="flex flex-col space-y-6">
			...
			<div>
				<h2 className="text-lg font-bold">
					{locationLookupMessages.en.locationLookupSearchResults}
				</h2>
...
export const AddressResults = ({
	...
	const renderSearchResults = () => (
		<div className="grid grid-cols-2 gap-4 mt-6">
			{searchResults
				.slice(0, 6)
				.map((result: SearchResultsTypes, index: number) => (
					<div
						key={index}
						className="flex flex-col space-y-2.5 col-span-2 md:col-span-1"
					>
						<button
							className={clsx("secondaryBtn flex-col flex", {
								"active border border-dashed border-blue-600":
									selectedLocation?.formatted === result.formatted,
							})}
		...
		<div className="w-full">
			<div className="w-full flex justify-end">
				<button
					...
					className="secondaryBtn flex gap-x-3 items-center"
				>
					...
					<div className="mx-4 px-4 py-10 bg-gray-800 border border-gray-700 space-y-6">
						<div>
							<h3 className="text-lg font-bold">
								{locationLookupMessages.en.lookupLocationTitle}
							</h3>
							<p className="text-sm">
								{locationLookupMessages.en.lookupLocationDescription}
							</p>
						</div>
						<LocationLookupForm />
					</div>
