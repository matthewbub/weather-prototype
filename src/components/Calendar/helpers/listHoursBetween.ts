...
const parseTime = (timeString: string) => {
	...
	const hour24 =
		isPM && hourNumber !== magicNumber("12")
			? hourNumber + magicNumber("12")
			: hourNumber === magicNumber("12") && !isPM
				? magicNumber("0")
				: hourNumber;
...
const formatTime = (time: Date) => {
	...
	return `${hour12}:${minute
		.toString()
		.padStart(magicNumber("2"), "0")}${meridiem}`;
};
...
const listHoursBetween = (startRow: string, endRow: string) => {
	...
	while (currentHour <= endRowTime) {
		...
		currentHour = new Date(
			currentHour.getTime() +
				magicNumber("60") * magicNumber("60") * magicNumber("1000"),
		);
	}
