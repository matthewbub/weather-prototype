...
/** Components */
import { BaseDayComponent, BaseDayNameComponent } from "./components";
...
const Calendar: FC<CalendarProps> = ({
	...
	const createCalendarDays = (start: Date, end: Date) => {
		...
		for (let i = 1 - startWeekday; i <= totalDays + 6 - end.getDay(); i += 1) {
			...
			const customDate = safeCustomDates.find(({ date }) =>
				isSameDay(date, currentDate),
			);

			const dayComponent: JSX.Element | null =
				showAdjacentDays || (i > 0 && i <= totalDays) ? (
					<DayComponent
	...
	return (
		...
		>
			...
			<div className={customClassNames.component}>
				...
				<React.Fragment>{weeks}</React.Fragment>
			</div>
