 ...
 */
const DraggableDayLayout: FC<DraggablesProps> = ({
	startRow,
	endRow,
	initialDraggables,
}) => {
	...
	useEffect(() => {
		setDraggableDays(
			initialDraggables || [{ id: 1, position: { x: 0, y: 0 }, height: 1 }],
		);
	}, [initialDraggables]);
	...
	const deleteDraggable = (id: number) => {
		setDraggableDays(
			draggableDays.filter((draggableDay) => draggableDay.id !== id),
		);
	};
			...
			>
				<label>
					...
					<input
						...
						onChange={(e) =>
							setDefaultHeight(
								Math.max(1, Math.min(parseInt(e.target.value), dynamicRows)),
							)
						}
					/>
				</label>
