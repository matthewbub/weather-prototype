...
export const useMouseMove = (
	...
	return useCallback(
		(e: MouseEvent) => {
			...
			const rowHeight = containerRect
				? containerRect.height / dynamicRows
				: null;
			...
			if (containerRect && rowHeight) {
				if (dragging) {
					...
					const posY = Math.max(
						0,
						Math.round((e.clientY - containerRect.top) / quarterRow) *
							quarterRow,
					);
					...
					const quarterRow = rowHeight / 4;
					const newRowHeight =
						Math.round((posY - currentPos.y) / quarterRow) * quarterRow;
