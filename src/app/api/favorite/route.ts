...
const addUsersFavoriteLocation = async (
	userSpecificLocationId: string,
	userId: string,
) => {
	const { data: location, error: locationError } = await supabase
...
export async function POST(request: Request) {
	...
	const { error: postValidationErrorForFavoriteLocation } =
	...
	const location = await confirmLocationExistsForUser(
		requestData?.usersGeolocationId,
	);
	if (location?.error) {
