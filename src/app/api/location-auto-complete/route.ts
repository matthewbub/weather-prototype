		...
		case "city":
			try {
				const cityResponse = await fetch(
					`${baseUrl}?text=${cityName}&type=city&format=json&apiKey=${process.env.GEOAPIFY_KEY}`,
				);
				const data = await cityResponse.json();

				// remove items that aren't in the US
				const collection = data?.results.filter(
					(item: { country_code: string }) => item.country_code === "us",
				);
				const processedCollection = collection.map(
					({ lat, lon, formatted, city, state, county }: LocationTypes) => ({
						lat,
						lon,
						formatted,
						city,
		...
		case "postcode":
			try {
				const postcodeResponse = await fetch(
					`${baseUrl}?text=${postcode}&type=postcode&format=json&apiKey=${process.env.GEOAPIFY_KEY}`,
				);
